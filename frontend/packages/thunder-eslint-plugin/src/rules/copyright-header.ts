    ...
    return {
      Program(node: unknown) {
        ...
        if (!firstComment || firstComment.type !== 'Block') {
          context.report({
            ...
            fix(fixer: Rule.RuleFixer) {
              return fixer.insertTextBefore(node as Rule.Node, `${template}\n\n`);
            },
          });
          return;
        }
        ...
        if (!normalizedComment.includes('WSO2 LLC') || !normalizedComment.includes('Apache License')) {
          context.report({
            node: firstComment as unknown as Rule.Node,
            messageId: 'incorrectHeader',
            fix(fixer: Rule.RuleFixer) {
              return fixer.replaceText(firstComment as unknown as Rule.Node, template);
            },
          });
        }
      },
