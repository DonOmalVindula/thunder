...
/**
 * Sets the global class name prefix used by `tn()`.
 *
 ...
 * @example
 * ```ts
 * setTnPrefix(window.__THUNDER_RUNTIME_CONFIG__?.brand?.product_name ?? 'Thunder');
 * ```
 *
 * @param newPrefix - The prefix to use for all class names
 */
export function setTnPrefix(newPrefix: string): void {
  prefix = newPrefix;
}
 ...
 */
export function getTnPrefix(): string {
  return prefix;
}
 ...
 */
export default function tn(...classes: ClassValue[]): string {
  return classes
    .filter(Boolean)
