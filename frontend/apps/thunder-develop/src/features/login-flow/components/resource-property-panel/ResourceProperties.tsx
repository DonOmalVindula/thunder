...
import {ElementCategories, type Element} from '@/features/flows/models/elements';
...
function ResourceProperties({
  ...
  // Adapter to convert string-based onChange to unknown-based onChange
  const handleChange = (propertyKey: string, newValue: unknown, changedResource: unknown): void => {
    const stringValue = typeof newValue === 'string' || typeof newValue === 'number' ? String(newValue) : '';

    onChange(propertyKey, stringValue, changedResource as Resource);
  };
  const [selectedVariant, setSelectedVariant] = useState<Element | undefined>(() => {
    ...
    const currentVariant = resource.variants.find(
      (v: Element) => v.variant === (resource as Element).variant,
  ...
  switch (resource.category) {
    ...
    case ElementCategories.Field:
      return (
        <>
          {renderElementId()}
          {/* <FieldExtendedProperties resource={resource} onChange={onChange} /> */}
      ...
      return (
        <>
          ...
          {/* {resource.type === ElementTypes.Button && (
            <ButtonExtendedProperties resource={resource} onChange={onChange} onVariantChange={onVariantChange} />
          )} */}
          {renderElementPropertyFactory()}
        </>
      );
