...
import type {Resource as FlowResource, Resource} from '@/features/flows/models/resources';
...
function LoginFlowBuilder() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const handleResourceAdd = (_resource: FlowResource): void => {};
  ...
  const handleWidgetLoad = (
    ...
    const customMerge = (objValue: unknown, srcValue: unknown, key: string): Element[] | undefined => {
      ...
      if (key === 'components' && Array.isArray(objValue) && Array.isArray(srcValue)) {
        ...
        return unionWith(objValue as Element[], srcValue as Element[], isEqual);
      }
  ...
  const nodeTypes = useMemo((): NodeTypes => {
    ...
    const stepNodes: NodeTypes = steps.reduce((acc: NodeTypes, resource: Step) => {
      acc[resource.type] = (props: NodeProps) => (
        ...
        <StepFactory resourceId={props.id} resources={stepsByType[resource.type]} {...props} />
      );
